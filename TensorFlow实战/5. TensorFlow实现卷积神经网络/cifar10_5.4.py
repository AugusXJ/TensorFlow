...
print('-------------------------------------------')
max_steps = 3000                                            # 训练轮数
batch_size = 128                                            # 训练样本数量
data_dir = '/tmp/cifar10_data/cifar-10-batched-bin'         # 数据下载路径
...
norm1 = tf.nn.lrn(pool1, 4, bias=bias1, alpha=0.001/9.0, beta=0.75)      # LRN(局部响应归一化)层
...
# 第三层：全连接层
reshape = tf.reshape(pool2, [batch_size, -1])                   # 将图片尺寸reshape到一维向量
...
def loss(logits, labels):
    ...
    return tf.add_n(tf.get_collection('losses'), name='total_loss')                 # 返回最终loss
